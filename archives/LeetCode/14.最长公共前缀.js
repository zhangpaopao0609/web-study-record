...
function longestCommonPrefixDivide(strs, left, right) {
  if (left === right) {
    return strs[left];
  }
...
// 2.2 横向比较
function longestCommonPrefix_2(strs) {
  if (strs == null || strs.length === 0) {
    return '';
  }
  const len = strs.length;
...
// ! 归并的思想
function longestCommonPrefix_3(strs) {
  if (strs == null || strs.length === 0) {
    return '';
  }
  return longestCommonPrefixDivide(strs, 0, strs.length - 1);

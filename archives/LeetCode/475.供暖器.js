...
function findRadius(houses, heaters) {
  let max = 0; const hol = houses.length; const hel = heaters.length;
  let i = j = 0; let flag = false;
  ...
  if (flag) {
    max = Math.max(max, houses[hol - 1] - heaters[hel - 1]);
  }
  return max;
}
// @lc code=end
...
function findNearestHeater(heaters, house) {
